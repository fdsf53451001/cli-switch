...
fn exe_path() -> String {
    std::env::current_exe()
        ...
        .unwrap_or_else(|_| "agent-sync".to_string())
}
...
fn install_claude_hook(exe: &str) -> R<String> {
    ...
    // Drop any prior agent-sync SessionStart group, then add a fresh one.
    let mut groups: Vec<Value> = hooks
        .get("SessionStart")
        .and_then(|v| v.as_array())
        .cloned()
        .unwrap_or_default();
    groups.retain(|g| !group_mentions(g, "agent-sync"));
    groups.push(json!({
...
fn install_opencode_plugin(exe: &str) -> R<String> {
    ...
    let js = format!(
        r#"// agent-sync: auto-sync MCP/skills/instructions on opencode startup.
// Generated by `agent-sync mount`. Experimental: opencode plugin event names
...
fn write_shell_init(exe: &str) -> R<String> {
    ...
    let script = format!(
        r#"# agent-sync shell init — source this from ~/.zshrc or ~/.bashrc:
#   source "{path}"
...
__agent_sync_run() {{ command "{exe}" sync --quiet >/dev/null 2>&1 || true; }}
kiro()        {{ __agent_sync_run; command kiro "$@"; }}
agy()         {{ __agent_sync_run; command agy "$@"; }}
antigravity() {{ __agent_sync_run; command antigravity "$@"; }}
"#,
        path = path.display(),
        exe = exe
    );
